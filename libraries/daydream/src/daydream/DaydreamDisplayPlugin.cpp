...
/*    gpu::Batch extraBatch;
...

    {
        ...
        compositeExtra();// CLD what batch to use? extraBatch); CLD This does nothing now, same is happening in HMDDisplayPlugin
    }
    {
...
void DaydreamDisplayPlugin::prepareFrameBuffer() {
    ...
    _frame.BindBuffer(0);
    /*gpu::Batch auxBatch;
    auxBatch.setFramebuffer(gpu::FramebufferPointer());
    _gpuContext->executeBatch(auxBatch);*/
}
...
// CLD No more _handPoses in hmdDisplayPlugin?       _handPoses[1] = handPoses[1];
...
void DaydreamDisplayPlugin::daydreamGLSetup() {
