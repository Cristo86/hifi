...
void DeferredLightingEffect::init() {
    ...
    qDebug() << "DeferredLightingEffect::init adding Light" << (lp?1:0);
    qDebug() << "DeferredLightingEffect::init adding Light prev size: " << _lightStage.lights.size();
    _lightStage.addLight(lp);
    qDebug() << "DeferredLightingEffect::init after adding light size: " << _lightStage.lights.size();
    ...
    gpu::doInBatch(args->_context, [&](gpu::Batch& batch) {
        ...
        // Setup the global directional pass pipeline
        {
            if (deferredLightingEffect->_shadowMapEnabled) {
                qDebug() << "RenderDeferredSetup::run 001510";
                ...
                qDebug() << "RenderDeferredSetup::run 001520";
                if (keyLight->getAmbientMap()) {
